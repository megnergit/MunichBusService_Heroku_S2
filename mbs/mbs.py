...
def visualize_pie(df, size, text):
    ...
    overall_sentiment = df_yesterday['confidence_mean'].values[0]
    count_sent = df_yesterday[cols].values.flatten()
#    print(count_sent)
    ...
    layout = go.Layout(
        ...
        annotations=[
            ...
            dict(font=dict(size=40, color=overall_color),
                 text=f'{overall_sentiment:.2}',
                 showarrow=False,
                 x=0.5, y=0.52,
                 xanchor='center', yanchor='middle'),

            dict(font=dict(size=24, color='purple'),
...
#     df_agg = aggregate_sentiment_tz(df_pn, freq='12H')
    df_agg = aggregate_sentiment_tz(df_pn, freq='24H')
...
def visualize_agg(df, size):
    ...
    # use 'tickwidth' next time. 0.0-0.5
